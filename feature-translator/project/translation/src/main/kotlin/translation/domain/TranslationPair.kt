          ...
          workDataRepository.getWorkData()
            .onSuccess { currentWorkData ->
              ...
              workDataRepository.setWorkData(updatedWorkData)
                ...
                .onFailure { exception ->
                  setError("Failed to save translations: ${exception.message}")
                }
            }
            .onFailure { exception ->
              setError("Failed to load work data: ${exception.message}")
            }
        } else {
          textDataRepository.loadWorkData(project, TextType.UNTRANSLATED)
            .onSuccess { untranslatedWorkData ->
              if (untranslatedWorkData == null) {
                setError("No untranslated work data found")
                return@launch
              }
              val translatedWorkData = updateWorkDataWithTranslations(untranslatedWorkData)
              textDataRepository.saveWorkData(project, TextType.TRANSLATED, translatedWorkData)
                .onSuccess {
                  showSuccess("Translations saved successfully")
                }
                .onFailure { exception ->
                  setError("Failed to save translated work data: ${exception.message}")
                }
            }
            .onFailure { exception ->
              setError("Failed to load untranslated work data: ${exception.message}")
            }
        }
      } finally {
        setLoading(false)
